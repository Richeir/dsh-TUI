...
- 右组：git 分支 → PR（当前分支打开的 PR，`PR #123`，绿色独立配色，依赖 gh CLI 探测，探测不到则不显示；hover 查看 PR URL，全屏模式下点击直接在浏览器打开该 PR）→ 工作目录（紧凑模式仅 basename）→ 会话标题 → 短会话 ID（`#` + 前 8 位，与日志文件名对应，方便 `--resume` 定位）
...
dsh-tui 自身区块（写入 settings.yaml 用户层，实时生效）如下；`statusBar.*` 一行展开成十几个独立开关，**完整项数以 `/settings` 屏内所见为准**：
...
| statusBar.* | 上表全部状态栏开关（compact/model/thinking/cwd/contextUsage/cache/tokens/cost/tps/gitBranch/pullRequest/sessionTitle/sessionId/goal/mode/contextBar/activity/trajectory/shortcutHint；statusBar.pullRequest 需安装并登录 GitHub CLI（gh），启动与切换工作目录时探测当前分支的开放 PR，gh 缺失 / 未登录 / 无 PR / 超时一律留空，想知道原因就带 `DSH_TUI_DEBUG=1` 起（stderr 诊断，不影响渲染）；statusBar.sessionId 是底栏显示开关，与 cordis 的启动 sessionId 无关） |
