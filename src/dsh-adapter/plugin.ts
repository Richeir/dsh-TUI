    ...
    const unregister = settingsSections.register({
      ...
      fields: [
        ...
        },
        {
          ...
          hint: 'Show the open PR for the current branch (PR #123) in the footer. Needs the GitHub CLI (gh) installed and authenticated; probed on startup and when the workspace changes, silently blank otherwise.',
          hintDescriptions: { zh: '在底部状态栏显示当前分支打开的 PR（PR #123）。需要安装并登录 GitHub CLI（gh）；启动与切换工作目录时探测，探测不到则不显示。' },
          group: 'status-bar',
          kind: 'boolean',
        },
        {
